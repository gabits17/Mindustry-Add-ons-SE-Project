...
## Choose Copy
![Choose Copy.svg](assets/Choose Copy.svg)
...
When pressing the ``Ctrl``+ ``V``, the player notifies the ``Input`` class and, per consequence, the ``DesktopInput``. In ``DesktopInput``, it's checked if the History of copies is empty, via ``isEmpty()``. If false, the player can choose to copy the current visible Copy, by calling ``getCurrent()``, which is developed in the ``CopyHistClass``, or to navigate between the saved copies, a behaviour described later on in this report. If true, ``DesktopInput`` communicates with the ``UI``, in order to display the message ``Nothing to paste!``.


## Copy Schematic
![Copy Schematic.svg](assets/Copy Schematic.svg)
...
## Get next
![Get next.svg](assets/Get next.svg)
...
## Select tiles
![Select Tiles.svg](assets/Select Tiles.svg)

Similar to the previous Sequence diagram, Select tiles leads with an already existent functionality, so it's simplified in order to not deviate from the main goal.
